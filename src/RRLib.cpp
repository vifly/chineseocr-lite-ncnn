...
//----------------------------------------------
    void getQuadrangleSubPix_8u32f_CnR(const uchar *src, size_t src_step,
                                       cv::Size src_size, float *dst,
                                       size_t dst_step, Size win_size,
        ...
        for (y = 0; y < win_size.height; y++, dst += dst_step) {
            ...
            if ((unsigned) (cvFloor(xs) - 1) < (unsigned) (src_size.width - 3) &&
                ...
                for (x = 0; x < win_size.width; x++) {
                    ...
                    float a = (float) (xs - ixs), b = (float) (ys - iys),
                            a1 = 1.f - a, b1 = 1.f - b;
                    ...
                    if (cn == 1) {
                        ptr += ixs;
                        dst[x] = ptr[0] * w00 + ptr[1] * w01 + ptr[src_step] * w10 +
                                 ptr[src_step + 1] * w11;
                    } else if (cn == 3) {
                        ptr += ixs * 3;
                        float t0 = ptr[0] * w00 + ptr[3] * w01 +
                                   ptr[src_step] * w10 + ptr[src_step + 3] * w11;
                        float t1 = ptr[1] * w00 + ptr[4] * w01 +
                                   ptr[src_step + 1] * w10 +
                                   ptr[src_step + 4] * w11;
                        float t2 = ptr[2] * w00 + ptr[5] * w01 +
                                   ptr[src_step + 2] * w10 +
                                   ptr[src_step + 5] * w11;
                ...
                for (x = 0; x < win_size.width; x++) {
                    ...
                    if ((unsigned) iys < (unsigned) (src_size.height - 1))
                        ...
                        ptr0 = ptr1 =
                                src + (iys < 0 ? 0 : src_size.height - 1) * src_step;

                    if ((unsigned) ixs < (unsigned) (src_size.width - 1)) {
                        ptr0 += ixs * cn;
                        ptr1 += ixs * cn;
                        for (k = 0; k < cn; k++)
                            dst[x * cn + k] = ptr0[k] * w00 + ptr0[k + cn] * w01 +
                                              ptr1[k] * w10 + ptr1[k + cn] * w11;
                    } else {
...
//----------------------------------------------------------
    void getRotRectImg(cv::RotatedRect rr, Mat &img, Mat &dst) {
        if (dst.empty() || dst.cols != rr.size.width ||
            dst.rows != rr.size.height) {
...
//----------------------------------------------------------
    void copyToRotRectImg(cv::Rect src_roi, cv::RotatedRect rr, Mat &src,
                          Mat &dst) {
        ...
        warpAffine(tmp, dst, m, rs, INTER_CUBIC, BORDER_TRANSPARENT,
                   Scalar::all(0));
    }
...
//----------------------------------------------------------
    void copyToRotRectImg(cv::RotatedRect rr, Mat &src, Mat &mask, Mat &dst) {
        ...
        Mat dst_1 = dst.clone();
        warpAffine(tmp, dst_1, m, rs, INTER_CUBIC, BORDER_TRANSPARENT,
                   Scalar::all(0));
        Mat dst_m = Mat::zeros(dst.size(), CV_8UC1);
        warpAffine(tmp_m, dst_m, m, rs, INTER_CUBIC, BORDER_TRANSPARENT,
                   Scalar::all(0));
...
//----------------------------------------------------------
    void drawRR(cv::Mat &img, cv::RotatedRect rr, Scalar color, int width) {
        ...
        rr.points(rect_points);
        for (int j = 0; j < 4; j++) {
            line(img, rect_points[j], rect_points[(j + 1) % 4], color, width,
                 cv::LINE_AA);
        }
    }
} // namespace RRLib