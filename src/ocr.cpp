...
OCR::OCR(std::string model_path) {
    std::cout << "start load psenet" << std::endl;
    psenet.load_param((model_path + "/psenet_lite_mbv2.param").c_str());
    psenet.load_model((model_path + "/psenet_lite_mbv2.bin").c_str());

    std::cout << "start load crnn" << std::endl;
#if CRNN_LSTM
    crnn_net.load_param((model_path + "/crnn_lite_lstm_v2.param").c_str());
    crnn_net.load_model((model_path + "/crnn_lite_lstm_v2.bin").c_str());
    crnn_vertical_net.load_param((model_path + "/crnn_lite_lstm_vertical.param").c_str());
    crnn_vertical_net.load_model((model_path + "/crnn_lite_lstm_vertical.bin").c_str());
#else
    crnn_net.load_param((model_path + "/crnn_lite_dw_dense.param").c_str());
    crnn_net.load_model((model_path + "/crnn_lite_dw_dense.bin").c_str());
    crnn_vertical_net.load_param(
        (model_path + "/crnn_lite_dw_dense_vertical.param").c_str());
    crnn_vertical_net.load_model(
        (model_path + "/crnn_lite_dw_dense_vertical.bin").c_str());
#endif

    std::cout << "start load shufflenet" << std::endl;
    angle_net.load_param((model_path + "/shufflenetv2_05_angle.param").c_str());
    angle_net.load_model((model_path + "/shufflenetv2_05_angle.bin").c_str());

    // load keys
    ifstream in(model_path + "/keys.txt");
    std::string filename;
    std::string line;
...
std::vector<std::string> crnn_decode(const ncnn::Mat score,
    ...
    for (int i = 0; i < score.h; i++) {
        ...
        if (max_index > 0 && (not(i > 0 && max_index == last_index))) {
            ...
            //            std::string temp_str =
            //            utf8_substr2(alphabetChinese,max_index - 1,1)  ;
...
cv::Mat draw_bbox(cv::Mat &src,
                  const std::vector<std::vector<cv::Point>> &bboxs) {
    cv::Mat dst;
    if (src.channels() == 1) {
    ...
    for (int c = features.c - 1; c >= 0; --c) {
        ...
        for (int i = 0; i < features.h; i++) {
            for (int j = 0; j < features.w; j++) {
                ...
                if (c == features.c - 1)
                    scores.at<float>(i, j) =
                            srcdata[i * features.w + j +
                                    features.w * features.h * c];

                if (srcdata[i * features.w + j + features.w * features.h * c] >=
                    _thresh) {
    ...
    for (int idx = features.c - 2; idx >= 0; --idx) {
        while (!queue.empty()) {
            ...
            for (int d = 0; d < 4; ++d) {
                ...
                if (_y < 0 || _y >= mask.rows)
                    continue;
                if (_x < 0 || _x >= mask.cols)
                    continue;
                if (kernels[idx].at<uint8_t>(_y, _x) == 0)
                    continue;
                if (mask.at<int32_t>(_y, _x) > 0)
                    continue;
...
{
    // 翻转模式，flipCode == 0 垂直翻转（沿X轴翻转），flipCode > 0 水平翻转（沿Y轴翻转），flipCode < 0 水平垂直翻转（先沿X轴翻转，再沿Y轴翻转，等价于旋转180°）
    flip(src, src, 0);
    flip(src, src, 1);
    return src;
}
...
void OCR::detect(cv::Mat im_bgr, int long_size) {
    auto im = im_bgr;
    if (long_size > 0) {
        im = resize_img(im_bgr, long_size);
    }
    ...
    pse_decode(preds, contoursMap, 0.7311, 10, 1);
    ...
    for (auto &cnt: contoursMap) {
        ...
        for (int i = 0; i < bbox.rows; ++i) {
            points.emplace_back(cv::Point(int(bbox.at<float>(i, 0)),
                                          int(bbox.at<float>(i, 1))));
        }
        bboxs.emplace_back(points);
    }
    std::cout << "psenet decode 时间:"
              << (static_cast<double>(cv::getTickCount()) - time1) / cv::getTickFrequency()
              << "s" << std::endl;
    std::cout << "box size: " << bboxs.size() << std::endl;

    auto result = draw_bbox(im_bgr, bboxs);
    // cv::imwrite("./detect_box.jpg", result);
    ...
    for (int i = 0; i < rects.size(); i++) {
        ...
        // std::cout<<  temprect.size.width << "," << temprect.size.height <<
        // "," << temprect.center.x <<
        // "," <<  temprect.center.y  << "," <<  temprect.angle << std::endl;
        // cv::Mat part_im = crop_text_area(temprect,im_bgr);
        cv::Mat part_im;

        int min_size = temprect.size.width > temprect.size.height
        ...
        if (part_im_h > 1.5 * part_im_w)
            part_im = matRotateClockWise90(part_im);

        cv::Mat angle_input = part_im.clone();
        // part_im_w = angle_input.cols;
        // part_im_h = angle_input.rows;
        // std::cout << "网络输出尺寸 2(" << part_im_w<< ", " << part_im_h << ")"
        // << std::endl; cv::imwrite("test.jpg",part_im);
        //分类
        ncnn::Mat shufflenet_input = ncnn::Mat::from_pixels_resize(
                angle_input.data, ncnn::Mat::PIXEL_BGR2RGB, angle_input.cols,
                part_im.rows, shufflenetv2_target_w, shufflenetv2_target_h);
        ...
        // std::cout << "anglenet前向时间:" << (static_cast<double>(
        // cv::getTickCount()) - time2) / cv::getTickFrequency() << "s" <<
        // std::endl; std::cout << "网络输出尺寸 (" << preds.w << ", " <<
        // preds.h <<
        // ", " << preds.c << ")" << std::endl;
        ...
        ncnn::Mat crnn_in = ncnn::Mat::from_pixels_resize(
                img2.data, ncnn::Mat::PIXEL_BGR2GRAY, img2.cols, img2.rows,
                crnn_w_target, crnn_h);

        // ncnn::Mat crnn_in = ncnn::Mat::from_pixels_resize(part_im.data, ncnn::Mat::PIXEL_BGR2GRAY, part_im.cols, part_im.rows, crnn_w_target, crnn_h );

        crnn_in.substract_mean_normalize(mean_vals_crnn, norm_vals_crnn);

        ncnn::Mat crnn_preds;

        //判断用横排还是竖排模型 { 0 : "hengdao",  1:"hengzhen",  2:"shudao",
        ...
        if (angle_index == 0 || angle_index == 1) {
            ...
            ncnn::Mat blob182(256, blob162.h);
            for (int i = 0; i < blob162.h; i++) {
                ncnn::Extractor crnn_ex_1 =
                        crnn_vertical_net.create_extractor();
                crnn_ex_1.set_num_threads(num_thread);
                ncnn::Mat blob162_i = blob162.row_range(i, 1);
                crnn_ex_1.input("253", blob162_i);
        ...
        // std::cout << "前向时间:" << (static_cast<double>( cv::getTickCount())
        // - time1) / cv::getTickFrequency() << "s" << std::endl; std::cout <<
        // "网络输出尺寸 (" << crnn_preds.w << ", " << crnn_preds.h << ", " <<
        // crnn_preds.c << ")" << std::endl;
    ...
    std::cout << "角度检测和文字识别总时间:"
              << (static_cast<double>(cv::getTickCount()) - time1) /
                 cv::getTickFrequency()
              << "s" << std::endl;
}
