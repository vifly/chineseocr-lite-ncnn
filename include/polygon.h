...
class Polygon {
public:
    Polygon(cv::Mat &boxPts, cv::Size side, float scaleX = 1,
            float scaleY = 1) {
        for (int row = 0; row < boxPts.rows; ++row) {
            ...
            if (row == 0)
                ...
                throw std::range_error("check boxPts mast be (4, 2)");
        }
        vec2f = {lt, rt, rb, lb};
        SortVertex();
    }
    ...
    void SortVertex() {
        ...
        std::vector<cv::Point2f> vertex(
                {vec2f[(minAxis + 0) % 4], vec2f[(minAxis + 1) % 4],
    ...
    std::vector<cv::Point2f> ToQuadROI() {
        ...
        auto height = (int) round(
                fmax(Distance(vec2f[0], vec2f[3]), Distance(vec2f[1], vec2f[2])));
        auto width = (int) round(
                fmax(Distance(vec2f[0], vec2f[1]), Distance(vec2f[2], vec2f[3])));
